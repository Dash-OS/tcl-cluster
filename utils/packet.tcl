...
proc ::cluster::packet::encode { payload } {
  ...
  if { [dict exists $payload filter] } {
    ...
    dict set prefix 12 [dict get $payload filter]
  ...
  if { [dict exists $payload error] } { 
    dict set packet_dict 13 [dict get $payload error]
  }
  ...
  if { [dict exists $payload tags] } { dict set packet_dict 10 [dict get $payload tags] }
  if { [dict exists $payload keepalive] } { 
    dict set packet_dict 11 [dict get $payload keepalive]
  }
    ...
    while {$active} {
      ...
      if { $active } {
        switch -- $id {
          ...
          10 { dict set result tags $data }
          11 { dict set result keepalive $data }
          12 { 
